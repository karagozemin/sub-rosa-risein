...
import { HORIZON_TESTNET_URL } from "./stellar";

export const SOROBAN_TESTNET_RPC_URL = "https://soroban-testnet.stellar.org";
export const CONTRACT_ID = (import.meta.env.VITE_RISEIN_CONTRACT_ID ?? "").trim();
...
export function isContractConfigured(): boolean {
  return StrKey.isValidContract(CONTRACT_ID);
}

function configuredContract(): Contract {
  if (!isContractConfigured()) {
    throw new Error(
      "Soroban contract is not configured. Deploy it to testnet and set VITE_RISEIN_CONTRACT_ID in apps/web/.env.local.",
    );
  }
...
function invocationTransaction(source: Awaited<ReturnType<typeof sourceAccount>>, operation: ReturnType<Contract["call"]>) {
...
async function invokeContract(
  ...
  if (
    ...
    throw new Error("Soroban RPC rejected the contract transaction.");
  }
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    if (result.status === rpc.Api.GetTransactionStatus.FAILED) {
      throw new Error("The Soroban contract transaction failed on testnet.");
    }
  }
...
async function simulateRead(
  ...
  if (!simulation.result) {
    throw new Error("The contract returned no state.");
  }
